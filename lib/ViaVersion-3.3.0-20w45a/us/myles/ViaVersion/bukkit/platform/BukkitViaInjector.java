...
public class BukkitViaInjector implements ViaInjector
{
    ...
    private void injectChannelFuture(final ChannelFuture future) throws Exception {
        try {
            ...
            try {
                final ChannelInitializer<SocketChannel> oldInit = (ChannelInitializer<SocketChannel>)ReflectionUtil.get(bootstrapAcceptor, "childHandler", ChannelInitializer.class);
    ...
    public void uninject() {
        for (final ChannelFuture future : this.injectedFutures) {
            ...
            for (final String name : names) {
                final ChannelHandler handler = future.channel().pipeline().get(name);
                try {
                    final ChannelInitializer<SocketChannel> oldInit = (ChannelInitializer<SocketChannel>)ReflectionUtil.get(handler, "childHandler", ChannelInitializer.class);
            ...
            try {
                final ChannelInitializer<SocketChannel> oldInit2 = (ChannelInitializer<SocketChannel>)ReflectionUtil.get(bootstrapAcceptor, "childHandler", ChannelInitializer.class);
    ...
    public int getServerProtocolVersion() throws Exception {
        try {
            final Class<?> serverClazz = NMSUtil.nms("MinecraftServer");
            final Object server = ReflectionUtil.invokeStatic(serverClazz, "getServer");
            final Class<?> pingClazz = NMSUtil.nms("ServerPing");
            Object ping = null;
