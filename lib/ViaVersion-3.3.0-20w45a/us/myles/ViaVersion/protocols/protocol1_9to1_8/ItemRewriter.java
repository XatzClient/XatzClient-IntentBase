    ...
    public static void toClient(final Item item) {
        if (item != null) {
            if (item.getIdentifier() == 383 && item.getData() != 0) {
                ...
                final String entityName = ItemRewriter.ENTTIY_ID_TO_NAME.get((int)item.getData());
                if (entityName != null) {
                    final StringTag id = new StringTag("id", entityName);
                    entityTag.put(id);
                    tag.put(entityTag);
                }
                item.setTag(tag);
            ...
            if (item.getIdentifier() == 373) {
                ...
                final String name = potionNameFromDamage(item.getData());
                final StringTag potion = new StringTag("Potion", "minecraft:" + name);
                tag.put(potion);
                item.setTag(tag);
                item.setData((short)0);
            }
            if (item.getIdentifier() == 387) {
                ...
                if (pages == null) {
                    pages = new ListTag("pages", (List<Tag>)Collections.singletonList(new StringTag(Protocol1_9To1_8.fixJson("").toString())));
                    tag.put(pages);
                    item.setTag(tag);
                    return;
                }
