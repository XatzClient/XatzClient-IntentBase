...
public class Chunk1_8Type extends PartialType<Chunk, ClientWorld>
{
    ...
    public Chunk1_8Type(final ClientWorld param) {
        super((Object)param, (Class)Chunk.class);
    }
    ...
    public Chunk read(final ByteBuf input, final ClientWorld world) throws Exception {
        ...
        for (int i = 0; i < 16; ++i) {
            if ((bitmask & 1 << i) != 0x0) {
                sections[i] = (ChunkSection)Chunk1_8Type.CHUNK_SECTION_TYPE.read(input);
            }
        }
        for (int i = 0; i < 16; ++i) {
            if ((bitmask & 1 << i) != 0x0) {
        ...
        return (Chunk)new Chunk1_8(chunkX, chunkZ, groundUp, bitmask, sections, biomeData, (List)new ArrayList());
    }
    ...
    public void write(final ByteBuf output, final ClientWorld world, final Chunk chunk) throws Exception {
        ...
        for (int i = 0; i < chunk.getSections().length; ++i) {
            if ((chunk.getBitmask() & 1 << i) != 0x0) {
                Chunk1_8Type.CHUNK_SECTION_TYPE.write(buf, (Object)chunk.getSections()[i]);
            }
        }
        for (int i = 0; i < chunk.getSections().length; ++i) {
            if ((chunk.getBitmask() & 1 << i) != 0x0) {
